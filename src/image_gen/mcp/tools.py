...
def register(mcp: FastMCP) -> None:
    ...
    async def generate_image(
        ...
        # INTEGRATION: Track B renames the generation backend from `gemini` to
        # `provider_registry`; update this name list to match when merging.
        missing = _missing_services(app, "gemini", "storage", "quota", "db")
        if missing:
        ...
        try:
            # INTEGRATION (merge boundary): Track B swaps this generation CALL to
            # the provider registry (e.g. app.state.provider_registry[provider]
            # .generate_image(...)) and adds a `provider` param. Leave the call
            # itself to B; the surrounding error categorization below is Track C's.
